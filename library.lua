...
function Library:CreateWindow(...)
    ...
    if typeof(Config.Size) ~= 'UDim2' then Config.Size = UDim2.fromOffset(750, 650) end
    ...
    local SidebarButtonsContainer = Library:Create('Frame', {
        BackgroundTransparency = 1;
    ...
    Library:Create('UIListLayout', {
        ...
        FillDirection = Enum.FillDirection.Vertical;
        SortOrder = Enum.SortOrder.LayoutOrder;
        Parent = SidebarButtonsContainer;
    });
    ...
    function Window:AddTab(Name)
        ...
        -- Associate tab with current category if one exists
        if Window.CurrentCategory then
            table.insert(Window.CurrentCategory.Tabs, Tab);
        end;
    ...
    function Window:AddCategory(Name)
        ...
        local CategoryButtonOuter = Library:Create('Frame', {
            BackgroundColor3 = Library.BackgroundColor;
            BorderColor3 = Library.OutlineColor;
            BorderMode = Enum.BorderMode.Inset;
        ...
        Library:AddToRegistry(CategoryButtonOuter, {
            BackgroundColor3 = 'BackgroundColor';
            BorderColor3 = 'OutlineColor';
        });

        local CategoryButtonInner = Library:Create('Frame', {
            BackgroundColor3 = Library.BackgroundColor;
            BorderColor3 = Color3.new(0, 0, 0);
            BorderMode = Enum.BorderMode.Inset;
            Size = UDim2.new(1, -2, 1, -2);
            Position = UDim2.new(0, 1, 0, 1);
            ZIndex = 2;
            Parent = CategoryButtonOuter;
        });
        Library:AddToRegistry(CategoryButtonInner, {
            BackgroundColor3 = 'BackgroundColor';
        });
        ...
        local CategoryLabel = Library:CreateLabel({
            ...
            ZIndex = 3;
            Parent = CategoryButtonInner;
        });

        local CategoryIndicator = Library:Create('Frame', {
            BackgroundColor3 = Library.AccentColor;
            BorderSizePixel = 0;
        ...
        Library:AddToRegistry(CategoryIndicator, {
            BackgroundColor3 = 'AccentColor';
        });

        function Category:SelectCategory()
            -- Hide all other categories
            for _, OtherCategory in pairs(Window.Categories) do
                OtherCategory:DeselectCategory();
            end;

            -- Show this category's tabs
            Window.CurrentCategory = Category;
            CategoryIndicator.Visible = true;
            CategoryButtonInner.BackgroundColor3 = Library.MainColor;
            Library.RegistryMap[CategoryButtonInner].Properties.BackgroundColor3 = 'MainColor';
            ...
            -- Show first tab of this category, or just pick the first tab overall if no category tabs
            if #Category.Tabs > 0 then
                Category.Tabs[1]:ShowTab();
            elseif #Window.Tabs > 0 then
                -- If no tabs are in this category yet, show the first tab overall
                for _, Tab in pairs(Window.Tabs) do
                    Tab:ShowTab();
                    break;
                end;
        ...
        function Category:DeselectCategory()
            CategoryIndicator.Visible = false;
            CategoryButtonInner.BackgroundColor3 = Library.BackgroundColor;
            Library.RegistryMap[CategoryButtonInner].Properties.BackgroundColor3 = 'BackgroundColor';
        end;

        CategoryButtonOuter.InputBegan:Connect(function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) then
    ...
    function Window:GetCategory(Name)
    ...
    -- Auto-select first category on window creation
    local FirstCategory;
    for _, Category in pairs(Window.Categories) do
